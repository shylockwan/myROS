...
EstablishMap_ros::EstablishMap_ros(ros::NodeHandle& nh) :
	...
	event_.cur_state = EstablishMapEvent::ready;
	event_.next_state = EstablishMapEvent::ready;
	event_.map_state = 0;
	report_=event_.report_=0;
	cmd_from=0;
	repo_srv.request.id=2;
...
bool EstablishMap_ros::onInit() {
	...
	map_ = new Map("map","qr","qrcoordinate");
...
void EstablishMap_ros::stateUpdate() {
	if (ros::ok()) {
		if (flag_enable && event_.flag_stateswitch) {
		//	ROS_INFO("stateUpdating.......");
			...
			case EstablishMapEvent::ready:
				if (event_.cur_state != EstablishMapEvent::ready) { //stateChange
					se_ = this->ready_se;
					event_.cur_state = EstablishMapEvent::ready;
			...
			case EstablishMapEvent::logmap:
				if (event_.cur_state != EstablishMapEvent::logmap) { //stateChange
					se_ = this->logmap_se;
					StateLogMap* temp_ptr =dynamic_cast<StateLogMap*>(logmap_se);
					mu_.lock();
					temp_ptr->getImg(img_);
					mu_.unlock();
					event_.cur_state = EstablishMapEvent::logmap;
					ROS_INFO(
			...
			case EstablishMapEvent::wait_selfloc:
				if (event_.cur_state != EstablishMapEvent::wait_selfloc) { //stateChange
					se_ = this->waitloc_se;
					event_.cur_state = EstablishMapEvent::wait_selfloc;
					ROS_INFO(
			...
			case EstablishMapEvent::wait_ap:
				if (event_.cur_state != EstablishMapEvent::wait_ap) { //stateChange
					se_ = this->waitauto_se;
					event_.cur_state = EstablishMapEvent::wait_ap;
					ROS_INFO(
			...
			case EstablishMapEvent::qrscan:
					if (event_.cur_state != EstablishMapEvent::qrscan) { //stateChange
						se_ = this->qrscan_se;
						StateQrScan* temp_ptr =dynamic_cast<StateQrScan*>(qrscan_se);
						mu_.lock();
						temp_ptr->getImg(img_);
						mu_.unlock();
						event_.cur_state = EstablishMapEvent::qrscan;
						ROS_INFO(
...
void EstablishMap_ros::Report() {
	...
	case 3:
		...
		break;
	case 4:
		ROS_INFO("__________no img or State Init Fail__________");
		repo_srv.request.srv_result=1;
		break;
	case 5:
	...
	//2=establishmap
	//999=testport
	std::cout<<"cmd_from:  "<<cmd_from<<std::endl;
	switch (cmd_from) {
	case 999:
		callForReport("/map_test/report");
...
void EstablishMap_ros::disable() {
	std::cout<<"****************Node Disable****************"<<std::endl;
	flag_enable = 0;
	se_ = ready_se;
	event_.cur_state=EstablishMapEvent::ready;
	event_.next_state=EstablishMapEvent::ready;
...
;
void EstablishMap_ros::callForReport(const std::string& srvname)
{
...
void EstablishMap_ros::getRequestID(const int& id){
	cmd_from=id;
