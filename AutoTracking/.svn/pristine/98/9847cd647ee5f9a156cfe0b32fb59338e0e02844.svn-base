...
int aroundCenter(std::vector<cv::Point> ptset, const cv::Point2f& center_,
		float tolerance_ = 30);  //Hellen change 0912
...
//判断图像中是否存在轨迹线或者二维码
	...
	processImg(src);
	ROS_INFO("StateReady: bsize=%d , rsize=%d", ptset_QR.size(), ptset_line.size());
...
//循线移动
	...
	processImg(src);
	ROS_INFO("autopatrol  ready: bsize=%d , rsize=%d", ptset_QR.size(), ptset_line.size());
	...
	if (ptset_QR.size() < B_SIZE && ptset_line.size() <R_SIZE) {
	  ...
	  {
	  	std::cout << "autopatrol aaaaaaaaaaaaaaaaaaaa ptset_world_pre.size()=" <<ptset_world_pre.size()<< std::endl; 
			setTwist(calcObjTwist(ptset_pic, src.cols, src.rows,robot_status_));
		}	
		else{
			std::cout << "autopatrol bbbbbbbbbbbb " <<std::endl; 
			state_.next_state = AutoPatrolEvent::ready;
			this->flag_selfLocate = 0;
			std::cout << "autopatrol cccccccccccc state_.next_state = " <<state_.next_state<< std::endl; 			
		}
		return;
	} //if 1
	else	if (ptset_QR.size() > B_SIZE) {
		
		std::cout << "autopatrol d1d1d1d1d1d1 flag_selfLocate = " <<flag_selfLocate<< std::endl; 	
		...
		else //elseQR已经扫描
		{
		    ...
		    std::cout << "autopatrol dddddddd flag_selfLocate = " <<flag_selfLocate<< std::endl; 		
		    std::cout << "autopatrol d2d2d2d2 state_.next_route->id_to = " <<state_.next_route->id_to<<"  state_.next_route->id_from= "<<state_.next_route->id_from<< std::endl; 		
		    			    			
				if (state_.next_route->id_to == state_.next_route->id_from) { //未设置目标路径！
					std::cout << "autopatrol e1e1e1e1e1e1e1e1e1e1 Path not set" << std::endl;
					state_.flag_eventFinish = 1;
					state_.next_state = AutoPatrolEvent::ready;
					setTwist();
					return;
					...
					std::cout << "Autopatrol find n=" << connect_domain.size() << "paths"	<< std::endl;
					for (std::vector<std::vector<cv::Point> >::iterator it =
					  ...
					  std::cout << "Autopatrol fffffffffffffff  angle_c=" << angle_c<< std::endl;						
					...
					std::cout << "Autopatrol gggggggggggggg  flag_havePathSelect=" << flag_havePathSelect<< std::endl;		
...
//提取路径问题
	...
	//state_.flag_eventFinish = 0;
	if (src.empty()) {
		std::cout<<"StatePathSelect stateHandle 图片空"<<std::endl;
		return ;
	}
	processImg(src);
	ROS_INFO("StatePathSelect: bsize=%d , rsize=%d", ptset_QR.size(), ptset_line.size());
	...
	if (ptset_QR.size() > B_SIZE) {
		...
		if (1 == res_) {
			...
			string qr_id=qr_decoder.decode(src,angleQR_);
			sleep(1);

			std::cout<<"StatePathSelect decode qr_id="<<qr_id<<std::endl;
			std::cout<<"StatePathSelect maybe scanQR has been finished"<<std::endl;
			...
			if(adjust_counts!=0)
			{
				...
				//do sth to adjust;
				adjust_counts--;
				std::cout<<"调整机器人姿态,使处于正中,   adjust_counts="<<adjust_counts<<std::endl;
			}
			else
			{
				setTwist();
			...
			ap_->haveNotSelflocate();
                        std::cout<<"StatePathSelect pppppppppppppppppppp "<<std::endl;
		} else {
			ROS_INFO("wtf?!!!");
		}
	} else {
