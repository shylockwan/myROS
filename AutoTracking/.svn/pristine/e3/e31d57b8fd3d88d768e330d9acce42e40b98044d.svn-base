...
AutoPatrol_ros::AutoPatrol_ros(ros::NodeHandle& nh) :
		 nh_(nh) ,it_(nh),update_rate(3),process_rate(3),
...
while( ros::ok() &&!flag_eventFinish && flag_enable){
	...
	case AutoPatrolEvent::uturn:
		if (e_.cur_state != AutoPatrolEvent::uturn) { //stateChange
					se_ = this->uturn_se;
					e_.cur_state=AutoPatrolEvent::uturn;
					ROS_INFO("state transform to uturn");
				}
				//else do nothing and spin as before
				break;
	default:
