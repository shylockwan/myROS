...
void StateReady::stateHandle(const cv::Mat& src, AutoPatrolEvent& event,RobotStatus& robot) {
	...
	else if (ptset_line.size() < R_SIZE) {
		std::cout<<"  ptset_QR.size() :"<<ptset_QR.size() <<"     ptset_line.size():"<<ptset_line.size()<<std::endl;
		event.next_state = AutoPatrolEvent::ready;
		event.report_=2;
		setTwist();
	} else  { //发现轨迹线色域，未发现QR色域进入ap态
...
void StateAP::stateHandle(const cv::Mat& src, AutoPatrolEvent& event,RobotStatus& robot) {
	...
	processImg(src);
	theta_robot_world=robot.m_ThetaRobot;
	pt_robot_world=robot.m_PtRobot;
	...
	{//发现QR色域，进入loc态
		...
		std::cout<<"  ptset_QR.size() :"<<ptset_QR.size() <<"     ptset_line.size():"<<ptset_line.size()<<std::endl;
		if (ptset_pre.size() > 2000) {//进入ap转弯等待
...
geometry_msgs::Twist StateAP::calcObjTwist(std::vector<cv::Point> ptset,
	...
	float diffangle_=getDiffangle(obj_point,pt_robot);
	obj_t+=diffangle_*K1;
...
//	obj_t+=diffdist*K2;
	...
	if(file.is_open())
	{
//		file<<ros::Time::now().sec<<"."<<ros::Time::now().nsec<<"时 robot_point:  ("<<rs.m_PtRobot.x<<"  ,"<<rs.m_PtRobot.y<<")"<<
//				" 转换后偏差:  "<<diffangle_<<std::endl;    //message是程序中处理的数据

		cv::Point2f pt_world = frameFiled2World(obj_point,pt_robot_world,theta_robot_world,rs.g_Fuyang, rs.g_Height);
	    //std::cout<<"pt_robot_world:"<<pt_robot_world<<"theta_robot_world: "<<theta_robot_world<<std::endl;
	//	std::cout<<"pt_world:  "<<pt_world<<std::endl;
		file<<ros::Time::now().sec<<"."<<ros::Time::now().nsec
				<<"时 robot_point:  "<<pt_robot_world<<
				"   obj_point:  ("<<pt_world.x<<"  ,"<<pt_world.y<<")"<<std::endl;
	...
	std::cout<<"obj_t: "<<obj_t<<"  diffdist: "<<diffdist<<"     diffangle_ :"<<diffangle_<<std::endl;
	std::cout<<"obj_point:   ("<<obj_point.x<<"    ,"<<obj_point.y<<")      robot_point:  ("<<pt_robot.x<<"  ,"<<pt_robot.y<<")"<<std::endl;
...
float StateAP::getDiffangle(cv::Point2f pt_obj,cv::Point2f pt_robot){
	float result=0.0;
...
//			ptset_world.push_back(pt_world);
	...
	pt_robot=cv::Point2f(0.0,-robot.g_Height*tan(robot.g_Fuyang) );
...
//	}
	///////////////////////////////////////////
	if (doGetObjPt(ptset_fields, result, pt_robot) ) {
...
void StateAP::drawPath(std::vector<cv::Point2f>& ptset,const cv::Mat& param){
	...
	path.at<cv::Vec3b>(cv::Point(wh+ pt_robot.x,wh- pt_robot.y))[0]=0;
	path.at<cv::Vec3b>(cv::Point(wh+pt_robot.x,wh- pt_robot.y))[1]=255;
	path.at<cv::Vec3b>(cv::Point(wh+ pt_robot.x,wh- pt_robot.y))[2]=255;
    cv::circle(path,cv::Point(wh+pt_robot.x,wh- pt_robot.y),2,cv::Scalar(255,0,255),1);
    cv::circle(path,cv::Point(wh+ obj_point.x,wh- obj_point.y),2,cv::Scalar(255,0,255),1);
	cv::imshow("path",path);
	//cv::waitKey(10);
	return;
}
...
void StateTurnAngle::stateHandle(const cv::Mat& src, AutoPatrolEvent& event,RobotStatus& robot) {
	...
	if(flag_set==0)
	{
		...
		float temp_angle=qr_angle+event.next_route->angle_to;
		temp_angle=(temp_angle > CV_PI?(temp_angle-(2*CV_PI)):temp_angle);
		temp_angle=(temp_angle < -CV_PI?(temp_angle+(2*CV_PI)):temp_angle);
		setObjAngle(temp_angle);
		std::cout<<"obj_angle:"<<obj_angle<<"  route_angle: "<<event.next_route->angle_to<<"  qr_angle:  "<<qr_angle<<std::endl;
		cur_theta=last_theta=robot.m_ThetaRobot;
		delta_angle=0;
		flag_set=1;
		return;
	}

	if(fabs(obj_angle)-delta_angle>0.1)
	{
        cur_theta = robot.m_ThetaRobot;
        float tmpdelta_angle= cur_theta-last_theta;
        tmpdelta_angle=(tmpdelta_angle> CV_PI? ((2*CV_PI)-tmpdelta_angle):tmpdelta_angle);
        tmpdelta_angle=(tmpdelta_angle< -CV_PI? ((2*CV_PI)+tmpdelta_angle):tmpdelta_angle);
        delta_angle += fabs(tmpdelta_angle);
	...
	{
		std::cout<<"_____________HHHHHHHHHHHHHHH stop  uturning ______________delta_angle is"<<delta_angle<<std::endl;
		setTwist();
		flag_set=0;
...
void StateQrcover::stateHandle(const cv::Mat& src,AutoPatrolEvent& event,RobotStatus& robot)
{
	...
	if(flag_set==0)
	{
		...
		{
			obj_dist=robot.g_Height*tan(robot.g_Fuyang)+5.0;
		...
		if(obj_dist-delta_dist>0.1)
		{
	        ...
	        delta_dist += ( fabs(cur_coor.x-last_coor.x)+fabs(cur_coor.y-last_coor.y) );
	        last_coor = cur_coor;
			setTwist(0.1,0,0,0,0,0);
			event.next_state=AutoPatrolEvent::qrcover;
		}
		else
		{
